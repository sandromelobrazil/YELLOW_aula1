    ...
    if '/' in _address:
        try:
            ...
            if option_args.vanilla:
                nmap_vanilla(network_address,'-F')
...
def run_threads(_wordlist,_number_threads):
    if number_threads >=2:
        with ThreadPoolExecutor(max_workers=_number_threads) as criathreads:
            for _number  in range(1,_number_threads):
...
##### Funcao UDP SCAN -sU
def nmap_udp(_target, _options):
    udpscan = pynmap_scan.nmap_udp_scan(_target,_options)
...
#    option.add_argument("-p",'--ports',  nargs='+', required=True, dest='ports',  help=msg_port)
    option.add_argument("-tr",'--threads', dest='nthreads', default=1,  help=msg_port)
    ...
    if option_args.file != None:
        print('TESTE')
        print('FILE:',option_args.file)
        print('THREADS:', option_args.nthreads)
        run_threads(option_args.file,int(option_args.nthreads))
